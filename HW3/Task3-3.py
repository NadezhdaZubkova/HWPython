...
if re.search("[а-яА-Я]", word):
    sum_points=sum([key for char in word for key, value in ru_dict.items() if char in value])
else:
    sum_points=sum([key for char in word for key, value in eng_dict.items() if char in value])
